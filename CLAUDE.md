...
- **Git state:** PRs #1–#5 all merged; `main` is current as of 2026-07-29. **No PR is standing open** — open a fresh one when the next batch of work is ready (don't assume one exists). Runbook URLs point at `main`, so they only work after a merge. Canva connector needs re-auth (optional).
