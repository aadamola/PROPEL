...
- **Instagram keyword automation LIVE-READY (2026-09-15):** 24-rule fast lane in front of the brain for Shalom Park — deterministic pre-approved answers on a match, fall-through to the Concierge CORE on a miss, **both lanes through the same guardrail**. Nine `escalate_only` rules (returns, refund, bank details, discount, litigation, completion, charges, agency, mortgage) **outrank every commercial rule**. The ₦95m condo price is deliberately withheld pending Collins. Rule table is **in-house only** — `clients/shalom-park/keywords.csv` (edit) + `keywords.md` (read); plan: `06-instagram-keyword-plan.md`. All seven campaign words wired (CONDO/LAND/SUMMER/CHAIRMAN/DUPLEX/INVESTMENT/DEVELOPER). **Test suite now 109 passing** (`node tools/test-all.js`). Gated on the Business Suite session **and** a nine-item facts-sheet addendum — the campaign brief and the signed sheet disagree on deposits (50% vs 70% vs ₦5m), a September "summer sale" contradicts two signed fields, and "capital growth" is banned outright. Audit: `clients/shalom-park/07-campaign-brief-audit.md`.
