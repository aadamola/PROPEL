...
- **Instagram keyword automation LIVE-READY (2026-09-15):** 24-rule fast lane in front of the brain for Shalom Park — deterministic pre-approved answers on a match, fall-through to the Concierge CORE on a miss, **both lanes through the same guardrail**. Nine `escalate_only` rules (returns, refund, bank details, discount, litigation, completion, charges, agency, mortgage) **outrank every commercial rule**. The ₦95m condo price is deliberately withheld pending Collins. Rule table is **in-house only** — `clients/shalom-park/keywords.csv` (edit) + `keywords.md` (read); plan: `06-instagram-keyword-plan.md`. All seven campaign words wired (CONDO/LAND/SUMMER/CHAIRMAN/DUPLEX/INVESTMENT/DEVELOPER). **Test suite now 113 passing** (`node tools/test-all.js`). **₦95m condo price released 2026-09-15** (Collins restated the figure he signed); 648 sqm plots and the ~6,738 sqm development parcel live, the parcel deliberately unpriced. **Promotions carry `promo_from`/`promo_until`/`promo_response` and retire themselves — an undated promotion cannot be compiled.** Three items open with Collins: promo dates · the 70%-vs-signed-50% deposit anomaly · investment wording (**"capital growth" is held — it is on the `must_not_say` list Collins himself signed and is hard-blocked by the guardrail**). Audit: `clients/shalom-park/07-campaign-brief-audit.md`. Gated on the Business Suite session.
