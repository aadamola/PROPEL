...
Canonical names — `apply-client-ledger.sh` generates the first; the other two are added by hand after Phase D:

```
META_VERIFY_TOKEN_SHALOM_PARK=<generated by apply-client-ledger.sh>
SHALOM_PARK_WABA_TOKEN=<permanent system user token>
SHALOM_PARK_WABA_PHONE_ID=<phone number id>
SHALOM_PARK_APP_SECRET=<Meta app secret — signs every webhook payload>
```
