...
- **Instagram keyword automation LIVE-READY (2026-09-15):** 24-rule fast lane in front of the brain for Shalom Park — deterministic pre-approved answers on a match, fall-through to the Concierge CORE on a miss, **both lanes through the same guardrail**. Nine `escalate_only` rules (returns, refund, bank details, discount, litigation, completion, charges, agency, mortgage) **outrank every commercial rule**. The ₦95m condo price is deliberately withheld pending Collins. Rule table: Google Sheet + `clients/shalom-park/keywords.csv`; plan: `clients/shalom-park/06-instagram-keyword-plan.md`. **Test suite now 103 passing** (`node tools/test-all.js`). Gated only on the Business Suite session.
