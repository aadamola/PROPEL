...
| **Flowise** | WATCH | Overlaps Dify; evaluate only if Dify fails the VPS test. |
| **Botpress** (open core, multi-channel) | WATCH | Strong conversational tooling but pricing gravity pulls to its cloud; our stack covers the need. Revisit if dialogue complexity outgrows n8n/Dify. |
| **"OpenClaw"** (2026's viral agentic assistant, ~380k stars, 50+ tool connectors) | **WATCH — do not deploy** | Star-count is not a QA pass. Autonomous agents with broad tool access are the opposite of our guardrail model (KB-grounded, narrow, escalating). Security posture unproven for client data under NDPA. Re-scan in 6 months. |
| **Evolution API / WAHA / OpenWA** | **REJECT for client production** (see §1) | ToS-violating transport = ban risk we cannot sell. |
| **Fish Speech / CosyVoice / OpenVoice / XTTS-v2** | WATCH | Bench candidates behind Chatterbox for the voice cost-down experiment. |
