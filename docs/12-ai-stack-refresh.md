...
| **"OpenClaw"** (2026's viral agentic assistant, ~380k stars, 50+ tool connectors) | **ADOPT — contained engine-room roles only** *(re-adjudicated 2026-07-12 on founder push — he was right)* | Deployed in the "Agent DMZ" (isolated, no client data, no send capability, human-QA'd outputs) for three roles: **Red Team Engine** attacking our own Concierge, **Market Intelligence Analyst** (public-data corridor sweeps), **Prospect Research Desk**. Full doctrine + tiered model policy: docs/13. **Client-facing autonomy remains banned** — unchanged flip condition. |
