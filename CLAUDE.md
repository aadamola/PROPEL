...
- **Instagram keyword automation LIVE-READY (2026-09-15):** 24-rule fast lane in front of the brain for Shalom Park — deterministic pre-approved answers on a match, fall-through to the Concierge CORE on a miss, **both lanes through the same guardrail**. Nine `escalate_only` rules (returns, refund, bank details, discount, litigation, completion, charges, agency, mortgage) **outrank every commercial rule**. The ₦95m condo price is deliberately withheld pending Collins. Rule table is **in-house only** — `clients/shalom-park/keywords.csv` (edit) + `keywords.md` (read); plan: `06-instagram-keyword-plan.md`. All seven campaign words wired (CONDO/LAND/SUMMER/CHAIRMAN/DUPLEX/INVESTMENT/DEVELOPER). **Test suite now 117 passing** (`node tools/test-all.js`). **All seven campaigns live** (CONDO/LAND/SUMMER/CHAIRMAN/DUPLEX/INVESTMENT/DEVELOPER). ₦95m condo price released · 648 sqm plots · ~6,738 sqm parcel, deliberately unpriced. **Promotional terms live with a review date, not an end date** — Shalom Park's campaigns run until sold out, so rules carry `promo_review_by`; an unreviewed promo reverts to the signed terms by itself (**re-confirm by 2026-10-15**: ₦5m condo entry deposit · 70% on the 4-bed). **`SP-INVEST` makes the investment case from delivered facts only** — no projection, percentage, guarantee or claim that value will rise, because that language is on the `must_not_say` list the client signed and is hard-blocked by the guardrail; `SP-ESC-ROI` outranks it on any forecast question. Audit: `clients/shalom-park/07-campaign-brief-audit.md`. Gated on the Business Suite session.
